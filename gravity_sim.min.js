var GravitySim=function(a,b){function f(){KeyMouseEventHandlers.init(q);q.oncontextmenu=function(){return!1};a.onresize=function(){n=q.width=a.innerWidth;p=q.height=a.innerHeight}}function h(){for(var a=0;a<k.length;a++)for(var b=0;b<l.length;b++)k[a].addForceFrom(l[b]);for(a=0;a<l.length;a++)for(b=0;b<k.length;b++)l[a].addForceFrom(k[b]);for(a=0;a<k.length;a++)k[a].updatePosition();for(a=0;a<l.length;a++)l[a].updatePosition();t();w();x();m();e();setTimeout(h,y)}function t(){var a=KeyMouseEventHandlers.getMousePosition();
if(KeyMouseEventHandlers.mouseButtonDown(KeyMouseEventHandlers.mouseCode.left))v||(v=!0,s=KeyMouseEventHandlers.getMousePosition());else if(v){v=!1;var b=Misc.dist(a,s);k.push(new GravityWell({position:{x:(s.x-n/2)/c,y:(s.y-p/2)/c},density:1,radius:b,fillStyle:"red",useGradient:!0}))}v&&(g.beginPath(),a=Misc.dist(a,s),g.arc(s.x,s.y,a,0,2*Math.PI),g.fill())}function w(){var a=KeyMouseEventHandlers.getMousePosition();KeyMouseEventHandlers.mouseButtonDown(KeyMouseEventHandlers.mouseCode.right)&&
l.push(new GravityWell({position:{x:(a.x-n/2)/c,y:(a.y-p/2)/c},density:1,radius:2,fillStyle:"black"}))}function x(){var a,b;KeyMouseEventHandlers.keyDown(KeyMouseEventHandlers.keyCode.left)?a=1:KeyMouseEventHandlers.keyDown(KeyMouseEventHandlers.keyCode.right)?a=-1:KeyMouseEventHandlers.keyDown(KeyMouseEventHandlers.keyCode.up)?b=1:KeyMouseEventHandlers.keyDown(KeyMouseEventHandlers.keyCode.down)&&(b=-1);a&&(k.forEach(function(b){b.position.x+=
a*d.panSpeed}),l.forEach(function(b){b.position.x+=a*d.panSpeed}));b&&(k.forEach(function(a){a.position.y+=b*d.panSpeed}),l.forEach(function(a){a.position.y+=b*d.panSpeed}))}function m(){KeyMouseEventHandlers.keyDown(KeyMouseEventHandlers.keyCode.pageUp)?c=c<d.maxScale?c+d.zoomSpeed*c:d.maxScale:KeyMouseEventHandlers.keyDown(KeyMouseEventHandlers.keyCode.pageDown)&&(c=c>d.minScale?c-d.zoomSpeed*c:d.minScale)}function e(){g.save();g.translate(n/2,p/2);g.scale(c,c);g.fillStyle=
